...
- **What changed:** Refreshed only the repo-template snapshot from `efeba25a0fcd27a4f365d3768bc5d4750d21cdd4` to reviewed PR #210 merge SHA `62cf1d01314d4ff555ac9fa878c4f9a7a2477b80`, self-applied the managed carry/start-task copies, and strengthened the existing startup global-update contract and self-apply coverage. Cleanup is now bound to the verified filesystem and Git-index receipt: divergent staged/unstaged inputs are rejected, and changed or recreated sources are preserved with recovery details instead of being overwritten.
- **Verification:** The provider range contains only repo-template PR #210. `node --test test/agentSelfApply.test.mjs test/writeAgentLifecycle.test.mjs test/refreshSnapshots.test.mjs test/agentLifecycleScripts.test.mjs test/globalUpdates.test.mjs` passed 53/53; `npm test` passed 771 total / 769 passed / 2 skipped / 0 failed; `npm run snapshots:verify`, `npm run agent:self-apply -- --check`, `npm run update-ecosystem-overview -- --check`, `npm run docs:render -- --check`, and `git diff --check` passed. Doc health reported 0 blocking findings and 8 advisory warnings.
