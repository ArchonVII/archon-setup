...
- **Issue/PR:** #389 / #390
- **Branch:** agent/codex/389-task-repair-audit-readiness-and-refresh
...
- **What changed:** Missing selected package-script entries now keep startup readiness incomplete even when unrelated agent scripts remain. Documentation execution tests fail on network access and use the existing offline gitignore fixture. The repo-template snapshot is pinned to `efeba25a0fcd27a4f365d3768bc5d4750d21cdd4`, root lifecycle copies are self-applied, and Archon Setup explicitly adopts the provider's deterministic line-ending policy without changing the generic optional `.gitattributes` ownership contract.
- **Verification:** Failing-first focused tests produced exactly the expected readiness and network failures; `node --test test/onboardAudit.test.mjs test/docSystemExecution.test.mjs` then passed 24/24. `node --test test/agentSelfApply.test.mjs test/foundationParity.test.mjs test/agentLifecycleScripts.test.mjs` passed 21/21, including 56/56 refreshed provider carry/EOL tests executed inside the snapshot. `npm run snapshots:verify` passed at `repoTemplate@efeba25` (135 files); `npm run agent:self-apply -- --check` and `npm run docs:render -- --check` passed; `npm test` passed 770 total / 768 passed / 2 skipped.
- **Propagation:** pending the separately reviewed refresh of existing Hudson Bend PR #383; issue #370 remains paused and untouched.
