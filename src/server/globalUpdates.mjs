  ...
  },
  {
    ...
    distribution: {
      ...
      body: [
        ...
        "- Use `npm run agent:start-task -- <issue> --carry <path...>` only for explicit in-repo task inputs. Every dirty path must be explicitly covered; copy and cleanup are bound to verified filesystem and Git-index state. Directory copies are staged and verified before promotion so modes are preserved and prior destination data remain recoverable on failure. Divergent index/worktree versions are rejected, detected changes or recreations fail without overwriting them and report recovery locations, and unrelated dirt still blocks startup. Do not edit either checkout until `agent:start-task` returns.",
