...
const GLOBAL_UPDATES = [
  ...
  },
  {
    ...
    source: [
      ...
      "ArchonVII/repo-template PR #197",
    ...
    distribution: {
      ...
      body: [
        ...
        "  ```",
        "",
        "- Before creating a PR, save the filled body to a temporary file outside the worktree, set `$bodyFile`, `$title`, and `$branch`, then validate it through the supported npm wrapper:",
        "",
        "  ```powershell",
  ...
  },
  {
    ...
    distribution: {
      kind: "agents-managed-block",
      capabilityIds: ["foundation.agents", "foundation.hooks"],
      targetPath: "AGENTS.md",
      protectedBranches: ["main", "master"],
  ...
  },
  {
    ...
    source: [
      ...
      "ArchonVII/repo-template PR #212",
      "ArchonVII/repo-template PR #214",
      "ArchonVII/archon-setup src/registry/features.json",
      "ArchonVII/archon-setup issues #352 and #354",
    ],
    agentInstruction:
      "Agents must treat the selected capability manifest as the source of startup requirements; do not copy a fixed path inventory into distributed policy.",
    ...
    distribution: {
      ...
      body: [
        "## Selection-Derived Agent Startup Baseline",
        "",
        ...
        "- Use `npm run agent:start-task -- <issue> --carry <path...>` only for explicit in-repo task inputs. Every dirty path must be explicitly covered; copy and cleanup are bound to verified filesystem and Git-index state. Directory copies are staged and verified before promotion; task copies are isolated from source hard links. Tracked cleanup requires Git 2.25 or newer, streams paths without command-line expansion, and restores captured POSIX source-directory modes. Divergent index/worktree versions are rejected, detected changes or recreations fail without overwriting them and report recovery locations, and unrelated dirt still blocks startup. Do not edit either checkout until `agent:start-task` returns.",
