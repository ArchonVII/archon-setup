...
import * as writeDocHealth from "../src/server/tasks/writeDocHealth.mjs";
import { DOC_HEALTH_FILES } from "../src/server/tasks/writeDocHealth.mjs";
import { normalizeSnapshotText, REPO_TEMPLATE_SNAPSHOT } from "../src/server/tasks/repoTemplateSnapshot.mjs";

async function makeTarget() {
  return mkdtemp(join(tmpdir(), "archon-dochealth-"));
}
...
test("apply installs the doc-health runner + spec matching the normalized snapshot", async () => {
  const target = await makeTarget();
  await writeDocHealth.apply(makeCtx(target));
  for (const file of DOC_HEALTH_FILES) {
...
test("check is needs-apply before and already-done after apply", async () => {
  const target = await makeTarget();
  assert.equal(await writeDocHealth.check(makeCtx(target)), "needs-apply");
  await writeDocHealth.apply(makeCtx(target));
  assert.equal(await writeDocHealth.check(makeCtx(target)), "already-done");
});

test("verify passes after apply", async () => {
  const target = await makeTarget();
  await writeDocHealth.apply(makeCtx(target));
  assert.deepEqual(await writeDocHealth.verify(makeCtx(target)), { ok: true });
});
...
test("check/verify catch a drifted managed file and apply repairs it", async () => {
  const target = await makeTarget();
  await writeDocHealth.apply(makeCtx(target));
  const file = "scripts/doc-health/lib.mjs";
  await writeFile(join(target, file), "// drifted by hand\n");
  assert.equal(await writeDocHealth.check(makeCtx(target)), "needs-apply");
  assert.equal((await writeDocHealth.verify(makeCtx(target))).ok, false);
  await writeDocHealth.apply(makeCtx(target));
...
test("apply is idempotent — re-apply leaves the files byte-identical", async () => {
  const target = await makeTarget();
  await writeDocHealth.apply(makeCtx(target));
  const first = await Promise.all(DOC_HEALTH_FILES.map((f) => readFile(join(target, f), "utf8")));
  await writeDocHealth.apply(makeCtx(target));
  const second = await Promise.all(DOC_HEALTH_FILES.map((f) => readFile(join(target, f), "utf8")));
...
test("apply records every installed file in the manifest", async () => {
  ...
  await writeDocHealth.apply(ctx);
  const paths = ctx.manifest.createdFiles.map((f) => f.path);
  for (const file of DOC_HEALTH_FILES) assert.ok(paths.includes(file), `${file} recorded`);
});

test("apply preserves doc-health spec YAML frontmatter while repairing baseline content", async () => {
  const target = await makeTarget();
  await mkdir(join(target, "docs", "agent-process"), { recursive: true });
  await writeFile(
    join(target, "docs", "agent-process", "doc-health.md"),
    "---\nsummary: Local doc health process\nstatus: active\n---\n\n# Old Doc Health\n\nRepo-local wiki metadata must survive repair.\n",
    "utf8"
  );

  await writeDocHealth.apply(makeCtx(target));

  const body = await readFile(join(target, "docs", "agent-process", "doc-health.md"), "utf8");
  assert.match(body, /^---\nsummary: Local doc health process\nstatus: active\n---\n\n# Doc Health/m);
  assert.doesNotMatch(body, /# Old Doc Health/);
  assert.equal(await writeDocHealth.check(makeCtx(target)), "already-done");
  assert.deepEqual(await writeDocHealth.verify(makeCtx(target)), { ok: true });
});
